<html>
                ...
                <div id='artistCarousel' class='carousel slide' data-ride='carousel'>
                    <div class='carousel-inner'>
                        ...
                        $artistSQL = "SELECT *, COUNT(artwork.user_id) as total 
                                      ...
                                      JOIN uuser ON uuser.user_ID = artwork.user_id
                                      GROUP BY artwork.user_id 
                        ...
                        if ($artistResult) {
                            ...
                            while ($row = mysqli_fetch_assoc($artistResult)) {
                                ...
                                echo "<img src='$artistpng' alt='$artistname' class='artist-avatar' />";
                                echo "</a>";
                                echo "</div>";
                                echo "<div class='col-md-6'>";
                                echo "<div class='carousel-caption'>";
                                echo "<h5 class='name'>$artistname</h5>";
                                echo "<div class='introduce'>$artistintroduce</div>";
                                echo "</div>";
                                echo "</div>";
                                echo "</div>";
                                echo "</div>";
    ...
    <section id='third'>
        ...
        <div class='row'>
        <?php 
            $artworkjpgSQL = "SELECT * FROM artwork where artwork_price is NOT null ORDER BY artwork_ID DESC LIMIT 3 ";
            ...
            <div class='col-md-4'>
                <div class='outer'>
                   <?php echo"<a href='artwork.php#".$artwork_class."'>";?>
                        <div class='upper'>
...
$artworkSQL = "SELECT * FROM artwork A JOIN exhibition E ON A.exhibition_ID=E.exhibition_ID where A.exhibition_ID in (SELECT exhibition_ID FROM exhibition)";
if(@$artworkResult = mysqli_query($link, $artworkSQL)){
    while($row = mysqli_fetch_assoc($artworkResult)){
        if($o_exhibition_ID!=$row['exhibition_ID']){
            $count++;
            $o_exhibition_ID=$row['exhibition_ID'];
        }
        else{
        }
    }
...
$oo_exhibition_ID=0;
if(@$artworkResult = mysqli_query($link, $artworkSQL)){
    echo "<div class='bd-example' style='max-width:80%; padding-left:300px;'>";
    echo "<div id='carouselExampleCaptions' class='carousel slide' data-ride='carousel'>";
    echo "<ol class='carousel-indicators'>";
    for($n=0;$n<=($count-1);$n++){
        if($n==0){
            echo "<li data-target='#carouselExampleCaptions' data-slide-to='$n' class='active'></li>";
        }
        else{
            echo "<li data-target='#carouselExampleCaptions' data-slide-to='$n'></li>";
        }
    }
    echo "</ol>";
    echo "<div class='carousel-inner'>";
    while($row = mysqli_fetch_assoc($artworkResult)){
        if($oo_exhibition_ID!=$row['exhibition_ID']){
            $img=$row['artwork_file'];
            $name=$row['exhibition_name'];
            $bio=$row['exhibition_introduce'];
            $start_time=$row['exhibition_start_time'];
            $end_time=$row['exhibition_end_time'];
            $oo_exhibition_ID=$row['exhibition_ID'];
            if($inner_count==0){
                echo "<div class='carousel-item active'>";
                echo "<img src='$img' class='d-block w-100' >";
                echo "<div class='carousel-caption d-none d-md-block'>";
                echo "<h5>$name</h5>";
                echo "<p><font color='white'>$start_time"." to "."$end_time</font></p>";
                echo "</div>";
                echo "</div>";
            }
            else{
